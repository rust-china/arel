...
impl<V> Rem<ActiveValue<V>> for ActiveValue<V>
where
    V: Into<Value> + Copy + PartialEq + Rem<V, Output = V> + Default,
{
    type Output = Self;
    fn rem(mut self, rhs: ActiveValue<V>) -> Self::Output {
        match &mut self {
...
impl<V> RemAssign<ActiveValue<V>> for ActiveValue<V>
where
    V: Into<Value> + Copy + PartialEq + Rem<V, Output = V> + Default,
{
...
/// let a: ActiveValue<i32> = arel::Set(1);
/// let b = a % 2;
/// assert_eq!(b, ActiveValue::Changed(1, Box::new(ActiveValue::NotSet)))
/// ```
impl<V> Rem<V> for ActiveValue<V>
where
    V: Into<Value> + Copy + PartialEq + Rem<V, Output = V> + Default,
{
    type Output = Self;
    fn rem(mut self, rhs: V) -> Self::Output {
        match &mut self {
...
impl<V> RemAssign<V> for ActiveValue<V>
where
    V: Into<Value> + Copy + PartialEq + Rem<V, Output = V> + Default,
{
