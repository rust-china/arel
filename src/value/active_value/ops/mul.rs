...
impl<V> Mul<ActiveValue<V>> for ActiveValue<V>
where
    V: Into<Value> + Copy + PartialEq + Mul<V, Output = V> + Default,
{
    type Output = Self;
    fn mul(mut self, rhs: ActiveValue<V>) -> Self::Output {
        match &mut self {
...
impl<V> MulAssign<ActiveValue<V>> for ActiveValue<V>
where
    V: Into<Value> + Copy + PartialEq + Mul<V, Output = V> + Default,
{
...
/// let b = a * 2;
/// assert_eq!(b, ActiveValue::Changed(8, Box::new(ActiveValue::NotSet)))
/// ```
impl<V> Mul<V> for ActiveValue<V>
where
    V: Into<Value> + Copy + PartialEq + Mul<V, Output = V> + Default,
{
    type Output = Self;
...
impl<V> MulAssign<V> for ActiveValue<V>
where
    V: Into<Value> + Copy + PartialEq + Mul<V, Output = V> + Default,
{
